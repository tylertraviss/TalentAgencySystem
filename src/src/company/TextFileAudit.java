...
public class TextFileAudit implements AuditStrategyInterface {
	...
	public void writeInformationToTextFile() throws IOException {
		...
		execute(); // Must write to information instance variable before creating the text file
					// with it
	...
	private void displayClientInformation() {
		...
		for (Client tempClient : talentAgencyCompany.getAllClients()) {
			information.add(tempClient.getId() + " | " + tempClient.getName() + " | Value: "
					+ tempClient.getAnnualRevenueGenerated() + "\n");
	...
	private void displayEmployeeInformation() {
		...
		for (Employee tempEmployee : talentAgencyCompany.getEmployees()) {
			information.add(
					tempEmployee.getId() + " | " + tempEmployee.getName() + " | " + tempEmployee.getJobTitle() + "\n");
	...
	private void displayFinancialSituation() {
		...
		information.add(
				"Average Client Commission: " + talentAgencyCompany.calculateAverageClientCommissionValue() + "\n");
	}
